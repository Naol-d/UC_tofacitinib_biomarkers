...
#SBATCH --nodes=1
#SBATCH --mem=30G
#SBATCH --output=/scratch/prj/bmb_tofacitinib/research_project/bulkRNA_alignment/scripts/slurms/diseased_alignments/batch_2/slurm-%A_%a.out


echo $SLURM_ARRAY_TASK_ID
# Gets the line position of a fastq name within the disease_samples_batch_1.txt based on the array iteration
Filename=`head disease_samples_batch_2.txt -n $SLURM_ARRAY_TASK_ID | tail -n 1`
echo $Filename
...
INPUT_DIR="${BASE_DATA_DIR}/originals/bulkRNA/bulkRNA_raw_data/disease/batch_2/"

R1="${INPUT_DIR}/${Filename}_1.fq.gz"
R2="${INPUT_DIR}/${Filename}_2.fq.gz"

# Directory for outputs
OUTPUT_DIR="${BASE_DATA_DIR}/outputs/bulkRNA_aligned_outputs/batch_2_disease/"
mkdir -p $OUTPUT_DIR
...
--outFileNamePrefix ${OUTPUT_DIR}/2_${Filename}_ \
