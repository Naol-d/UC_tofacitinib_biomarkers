...
#SBATCH --nodes=1
#SBATCH --mem=7G
#SBATCH --output=/scratch/prj/bmb_tofacitinib/research_project/scRNA_alignment/scripts/slurms/data_download/dataset1/public_data_download-%A_%a.out


echo $SLURM_ARRAY_TASK_ID
# Gets the line position of a fastq name within the filename.txt based on the array iteration
SRR=`head  dataset_1_ssr_list.txt -n $SLURM_ARRAY_TASK_ID | tail -n 1`
echo $SRR
...
SRA_DATA_DIR="$DATA_DIR/dataset_1/sra"
mkdir -p $SRA_DATA_DIR

OUTPUT_DIR="$DATA_DIR/dataset_1/d1_scRNA_raw_data"
mkdir -p $OUTPUT_DIR
